...
export function getOrCreateTenantProfile(tenantId: string): TenantProfile {
  let profile = tenantProfiles.get(tenantId);
  if (!profile) {
    profile = {
      tenantId,
      displayNameEn: tenantId === 'tnt_demo' ? 'Shelfline Demo Bank' : tenantId,
      displayNameAr: tenantId === 'tnt_demo' ? 'بنك شيلف لاين التجريبي' : tenantId,
    };
...
export function generateApiKeySecret(prefix = 'shelfline_demo'): string {
